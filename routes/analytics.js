  ...
  try {
    ...
    switch (type) {
      ...
      case 'trends':
        ...
        break;
      case 'real-time':
        result = await getRealTimeAttendance(school_id, date_from, date_to, grade, company_id, limit);  // ✅ ADD limit parameter
        break;
      default:
...
async function getRealTimeAttendance(schoolId, dateFrom, dateTo, grade = null, companyId = null, limit = null) {  // ✅ Add limit parameter
  ...
  try {
    ...
    console.log('🔍 Executing query with limit:', useLimit ? recordLimit : 'NO LIMIT');
    
    // Main attendance query
    const attendanceResult = await executeQuery(attendanceQuery, params, 60000);  // ✅ Increased timeout for large queries

    console.log(`✅ Loaded ${attendanceResult.recordset.length} attendance records`);
    ...
    // Summary query (always get full summary regardless of limit)
    ...
    const queryTime = Date.now() - startTime;
    console.log(`Real-time attendance completed in ${queryTime}ms - returned ${enhancedAttendance.length} records`);

    return {
      current_activity: enhancedAttendance,
      summary: {
        total_records: summary.total_records || 0,
        returned_records: enhancedAttendance.length,  // ✅ Show how many were actually returned
      ...
      filters_applied: {
        ...
        limit_applied: useLimit ? recordLimit : 'none'  // ✅ Show what limit was used
      }
    };
  } catch (error) {
